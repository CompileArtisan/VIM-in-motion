      ...
      switch (baseCmd) {
        case "ls":
          ...
          if (cwd === "~/levels") {
            const fileElements = files.map((file, idx) => {
              const stageNumStr = file.match(/stage-(\d+)\.level/);
              let color = "#d4d4d4"; // default unlocked
              if (stageNumStr) {
                ...
                if (completedStages.includes(stageId)) {
                  ...
                  if (prevDone) {
                    color = "#ffffff"; // white for unlocked
                  } else {
                    color = "#808080"; // grey for locked
                  }
                } else {
                  color = "#808080"; // locked
                }
              }
              return <span key={idx} style={{ color }}>{file}</span>;
            });
            newHistory.push(<div style={{ display: "flex", gap: "1.5rem", flexWrap: "wrap", width: "100%" }}>{fileElements}</div>);
          } else {
        ...
        case "cat":
          ...
          if (!catTarget) {
            ...
            } else if (resolvedPath === "~/README.org") {
              newHistory.push(
                "#+title: VIM in Motion",
                "",
                "* Modes",
        ...
        case "vim":
          ...
          if (!vimTarget) {
            ...
            if (resolvedPath === "~/README.md" || resolvedPath === "~/README.org") {
              ...
              if (isLocked) {
                newHistory.push(`vim: ${vimTarget}: Permission denied. Stage is locked.`);
              } else {
                newHistory.push(`Opening ${vimTarget}...`);
                if (onOpenFile) {
                  if (stageIdMatch) {
